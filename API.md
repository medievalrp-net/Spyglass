...
void logFactionClaim(Player claimant, Block flag, String factionName) {
    ...
    omni.record(new BlockUseRecord(
            ctx.id(),
...
void showRecentBreaksByPlayer(UUID playerId, CommandSender sender) {
    ...
    omni.query(request).thenAccept(result -> {
...
```java
omni.registerQueryParamHandler(new FactionParam());
```
...
```java
omni.registerFlagHandler(new FriendlyOnlyFlag());
```
...
```java
omni.registerDisplayRenderer("faction-claim", new FactionClaimRenderer());
```
...
```java
omni.registerRollbackEffectHandler(new FactionTerritoryHandler());
...
```java
Set<String> enabled = omni.enabledEvents();
