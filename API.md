...
Want users to type `/sg search faction=red` and have your plugin
...
Override how a specific event renders in `/sg search` output and
...
public final class FactionTerritoryHandler implements RollbackEffectHandler {
    ...
    public RollbackResult apply(RollbackEffect.Custom effect) {
        ...
        // Build the inverse so /sg undo can re-apply this rollback.
...
// or push into Spyglass's undo ledger via /sg rollback when
