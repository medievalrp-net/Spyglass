...
This plugin is a fresh install with no migration path from the legacy MPL Omniscience. See the sibling `Omniscience/` repo for historical reference only — v1's `DataEntry` collection in Mongo is untouched and can stay around for archival, but v2 writes to its own `EventRecords` collection and does not read v1 data.
