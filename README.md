...
Successor to the MPL-licensed [Omniscience](../Omniscience). Built clean-room from the architectural dissection in [`docs/analysis/`](docs/analysis/00-overview.md). MIT licensed.

## Status

`v1.0.0` — feature-complete release. Ships 34 event types, block + container + entity-NBT rollback (experimental), vanilla and FAWE WorldEdit capture, the `/omniv2 tool` inspection wand, the v1 → v2 migration command, and an automated regression harness against both v1 and v2.
...
4. Grant `omniscience2.use`, `omniscience2.search`, `omniscience2.rollback` to trusted roles. `omniscience2.admin` is needed for the migration command.

## Upgrading from v1

Operators coming from the original MPL Omniscience should:

1. **Stop** the server (or at least unload the v1 plugin).
2. Rename `Omniscience.jar` → `Omniscience.jar.disabled` in `plugins/`.
3. Drop `Omniscience2-1.0.0.jar` into `plugins/`.
4. Start the server. Omniscience2 boots with its own database (`Omniscience2.EventRecords` by default). v1 data is untouched.
5. From the console, run `omniv2 admin migrate-v1 --dry-run` to preview the translation counts.
6. Run `omniv2 admin migrate-v1` for the real migration. On the MedievalRP dev corpus (384k documents) this takes about 40 seconds. Progress prints every 10k docs.
7. Verify via `omniv2 search a:break t:30d -g` that historical data is queryable.
8. Once confident, delete `Omniscience.jar.disabled` and `Omniscience` from Mongo at your leisure. v1 retention TTL will cleanse it anyway.

The migration reuses the target Omniscience2 connection and resumes cleanly after interruption via `--resume`. Events deferred to future versions (bundle, some 1.20+ sub-types you may not have used) skip with a counted log line — they do not fail the migration.
