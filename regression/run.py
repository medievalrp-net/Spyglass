...
def run_case(rcon, case: Case) -> CaseResult:
    ...
    if case.v2:
        ...
        if case.v2_follow_up:
            ...
            time.sleep(0.8)
            parts.append(rcon.command("omniv2 page 1") or "")
        result.v2_output = "\n".join(parts)
    ...
    if case.v1:
        ...
        if case.v1_follow_up:
            ...
            time.sleep(0.8)
            parts.append(rcon.command("omni page 1") or "")
        result.v1_output = "\n".join(parts)
...
def _needs_follow_up(command: str) -> bool:
    return command.startswith("omniv2 search") or command.startswith("omniv2 rollback") or command.startswith("omniv2 restore")


def _needs_v1_follow_up(command: str) -> bool:
    return command.startswith("omni search") or command.startswith("omni rollback") or command.startswith("omni restore")
