...
tasks.shadowJar {
    archiveBaseName.set("Omniscience2")
...
tasks.test {
    ...
    // fast; run via `./gradlew :omniscience2:ingestBench` or
    // `./gradlew :omniscience2:clickhouseBench`.
    useJUnitPlatform {
...
tasks.register<Test>("clickhouseBench") {
    description = "Runs the ClickHouse vs Mongo benchmark for Omniscience2 (requires Docker)."
    group = "verification"
...
tasks.register<Test>("ingestBench") {
    description = "Runs the Omniscience v1-vs-v2 ingest throughput benchmark (requires Docker)."
    group = "verification"
    testClassesDirs = sourceSets["test"].output.classesDirs
    classpath = sourceSets["test"].runtimeClasspath
    useJUnitPlatform {
    ...
    maxHeapSize = "2g"
    systemProperty("org.slf4j.simpleLogger.defaultLogLevel", "warn")
    // Forward OMNI_BENCH_* system properties from the gradle invocation
    // into the test JVM so CI can override defaults without recompiling
    // (./gradlew :omniscience2:ingestBench -DOMNI_BENCH_SUSTAINED_SEC=60).
    for ((key, value) in System.getProperties()) {
        val name = key.toString()
        if (name.startsWith("OMNI_BENCH_")) {
            systemProperty(name, value.toString())
