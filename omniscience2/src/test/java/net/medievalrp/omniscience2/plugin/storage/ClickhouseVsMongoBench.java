 ...
 * {@code -DOMNI_BENCH_RECORDS=N}. Each query runs five times back to
...
class ClickhouseVsMongoBench {
    ...
    void setup() {
        ...
        Omniscience2Config.Database mongoConfig = new Omniscience2Config.Database(
                Omniscience2Config.Backend.MONGO,
                mongoUri,
                "OmniBench_" + System.nanoTime(),
                "EventRecords",
                stubClickHouseConfig());
        mongoStore = new MongoRecordStore(mongoConfig, new IndexManager());

        Omniscience2Config.ClickHouse chConfig = new Omniscience2Config.ClickHouse(
        ...
        int recordCount = Integer.parseInt(System.getProperty("OMNI_BENCH_RECORDS",
    ...
    private static Omniscience2Config.ClickHouse stubClickHouseConfig() {
    ...
    private List<EventRecord> generateRecords(int count, Instant start) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            EventRecord record = switch (event) {
                ...
                case "withdraw" -> new net.medievalrp.omniscience2.api.event.ContainerWithdrawRecord(
                ...
                case "join" -> new net.medievalrp.omniscience2.api.event.JoinRecord(
                        id, "join", occurred, expiresAt,
                        origin, source, loc, PLAYER_NAMES[playerIdx],
                        "127.0.0.1");
                case "quit" -> new net.medievalrp.omniscience2.api.event.QuitRecord(
