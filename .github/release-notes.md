Spyglass 1.1 introduces separate Minecraft builds from one shared source release.
...
Minecraft 1.21.x remains on the separate `maintenance/1.21.11` release line with Java 21 and InvUI 1.49. Do not install a modern 1.1 JAR on a 1.21 server.

- Fixes #359–#361: protect inspection wands, index exact-location queries, and apply supported configuration changes through `/sg reload`.
- Adds Minecraft-aware GitHub update checks to `/sg version` and notifications for operators. Set `updates.enabled=false` and restart to disable checks and notifications.
- Fixes #362–#367 on applicable targets: golden-dandelion age locks, sulfur-cube interactions, copper-golem slot transfers, sulfur-spike cascades, cushion lifecycle/rollback and straw-bed consumption/rollback.
- Builds and applicable isolated runtime checks passed on all three targets. See `docs/logging-fixes-validation.md` for tested paths and coverage limits. Minecraft 26.3 uses Paper build 38 alpha.
