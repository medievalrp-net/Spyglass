# Omniscience2 — Codex handoff plan (Phase 1)

## Context

Omniscience2 is a clean-room rewrite of the MPL-licensed Omniscience plugin for Paper 1.21.8. It's a forensic logging + rollback plugin for a Minecraft server, paired with a public API module that other in-house plugins (WhisperNet, Cauldron, etc.) integrate against.

The repo at `/Volumes/External-NVME/Documents/GitHub/MedievalRP/Omniscience2` contains:
- `docs/analysis/` — 11 markdown files dissecting the original Omniscience plugin. These are your **primary spec**. Start with `00-overview.md` and `10-modernization-hotspots.md`.
- `LICENSE` (MIT), `README.md`, `.gitignore`, empty git repo on `main`.

The original source is at `../Omniscience/`. **Do not open its source files** unless the dissection docs don't answer a factual question (e.g. the full list of event-name strings, the Mongo field names). Never copy implementation code. Always paraphrase, rename, and rewrite from the docs.

## Clean-room rules (non-negotiable)

1. All code is new. No copy-paste from `../Omniscience/`. Write from the dissection docs.
2. Rename everything: `DataEntry → EventRecord`, `OEntry → Recorder` + typed record constructors, `DataWrapper → gone` (Mongo POJO codec replaces it), `OmniCore → Omniscience2Plugin`, `OmniApi → Omniscience2Api`, etc.
3. Package prefix: `net.medievalrp.omniscience2.*` (never `.omniscience.`).
4. Command: `/omniv2` (aliases `/o2`, `/omniscience2`). Never `/omni`.
5. Mongo collection: `EventRecords`. Do not read/write `DataEntry` (v1's collection).
6. Plugin name in `plugin.yml`: `Omniscience2`. Separate `plugins/` entry from v1.
7. If the dissection docs don't cover a fact you need, it's fine to grep `../Omniscience/` for specific strings (event names, config keys, Mongo field names) — but not to read implementation code.
...
```
Omniscience2/
├── settings.gradle.kts              ← rootProject.name = 'Omniscience2', include 'api', 'plugin'
...
│   └── src/main/java/net/medievalrp/omniscience2/api/
│       ├── Omniscience2Api.java     ← service interface, ServicesManager-registered
│       ├── event/
...
│   └── src/
│       ├── main/
│       │   ├── java/net/medievalrp/omniscience2/plugin/
│       │   │   ├── Omniscience2Plugin.java    ← JavaPlugin
│       │   │   ├── config/
│       │   │   │   └── Omniscience2Config.java   ← record tree, loaded via Configurate
│       │   │   ├── storage/
...
│       │   │   └── api/
│       │   │       └── Omniscience2ApiImpl.java   ← registered via Bukkit ServicesManager
│       │   └── resources/
│       │       ├── plugin.yml                 ← name: Omniscience2, main: ..., depend: [], softdepend: []
│       │       ├── config.conf                ← default Configurate HOCON
│       │       └── messages.conf              ← MiniMessage templates
│       └── test/java/net/medievalrp/omniscience2/plugin/
...
5. **Service-based API surface.** `Omniscience2Api` is a plain interface registered via `Bukkit.getServicesManager().register(...)`. No static singletons. External plugins do `Bukkit.getServicesManager().load(Omniscience2Api.class)`.
...
8. **Virtual threads for the async queue drain.** `Thread.ofVirtual().name("omni2-drain").start(...)`. No Bukkit async scheduler for the drain thread. Keep listener `save()` calls O(1) via `LinkedBlockingDeque.offer` with a bounded capacity; on full, drop with a counter + warn.
...
Cloud annotation-based. Subcommands on `/omniv2` (aliases `/o2`, `/omniscience2`):
...
After `./gradlew shadowJar` produces `plugin/build/libs/Omniscience2-<version>.jar`:

1. Rename the existing `/Volumes/External-NVME/Documents/GitHub/MedievalRP/RP_Server/plugins/Omniscience.jar` → `Omniscience.jar.disabled` so it doesn't conflict.
2. Copy the new jar to `RP_Server/plugins/Omniscience2.jar`.
...
3. `Omniscience2.jar` built and manually verified via RCON.
