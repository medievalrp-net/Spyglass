 ...
 * the event-name field via {@link net.medievalrp.omniscience2.api.event.EventCatalog}.
 *
 * <p>Earlier versions ran one Mongo query per record type when no
 * event filter was present — up to 13 round trips for a plain
 * {@code /omni2 search t:1h -g}. The discriminator replaces that with
...
public final class MongoRecordStore implements RecordStore {
    ...
    public MongoRecordStore(Omniscience2Config.Database config, IndexManager indexManager) {
