...
- `/omni rollback` with a 10-second window looks back 10 s; if the last 2 s of events haven't been flushed yet, the rollback is incomplete.
...
The queue cap is configurable via `Omniscience2Config.queue.capacity` (default 100 000 in production; the bench uses 10 000 to make the overload scenario measurable in reasonable time). For MedievalRP's production workload:
...
| `OMNI_BENCH_V2_CAP` | 10 000 | v2 queue cap for the bench (production default: 100 000). |
