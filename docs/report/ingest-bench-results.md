# Omniscience v1 vs v2 — ingest throughput bench

Measured side-by-side comparison of the v1 and v2 write pipelines under three load shapes. Generated by [`IngestThroughputBench`](../../omniscience2/src/test/java/net/medievalrp/omniscience2/plugin/pipeline/IngestThroughputBench.java), gated on Docker, run via `./gradlew :omniscience2:ingestBench`. All numbers below come from the 2026-04-24 run against Mongo 7.0 in testcontainers, on the development host. Latencies are **submit-to-Mongo-ack** wall time per record (nanoTime at `recorder.record()` → nanoTime inside `save()` after `bulkWrite` returns).

Raw data: [`omniscience2/build/reports/ingest-bench.json`](../../omniscience2/build/reports/ingest-bench.json).

## The contract v2 must honour

**Every event that fires a listener reaches the store.** This was v1's behaviour and has been for as long as Omniscience has run on MedievalRP. It is not negotiable for v2. If an audit-log plugin silently drops events under load, operators cannot trust `/omni rollback` or `/omni inspect` — if the data isn't there, did the event not happen, or did the plugin lose it? That ambiguity breaks the tool.

So v2's `AsyncRecorder` queue is **unbounded**. `record()` never rejects. The configurable `queue-capacity` is a **warn threshold** — crossing it logs an operator warning so a Mongo backlog is visible before it matures into a real problem, but the queue keeps accepting records. See [`AsyncRecorder`](../../omniscience2/src/main/java/net/medievalrp/omniscience2/plugin/pipeline/AsyncRecorder.java)'s no-drop Javadoc for the full invariant and the two remaining loss scenarios (hard JVM death, shutdown flush deadline exhaustion — both same exposure as v1).
...
```
WARNING: Omniscience2 recorder queue depth 10002 (warn threshold 10000). No records dropped — queue is unbounded — but heap pressure grows with depth. Check Mongo reachability and drain latency.
WARNING: Omniscience2 recorder queue depth 20005 ...
WARNING: Omniscience2 recorder queue depth 40010 ...
WARNING: Omniscience2 recorder queue depth 80020 ...
WARNING: Omniscience2 recorder queue depth 160042 ...
```
...
./gradlew :omniscience2:ingestBench

# Crank up the sustained scenario for capacity planning:
./gradlew :omniscience2:ingestBench \
    -DOMNI_BENCH_SUSTAINED_SEC=60 \
    -DOMNI_BENCH_SUSTAINED_RATE=50000

# Crank up the burst scenario:
./gradlew :omniscience2:ingestBench \
    -DOMNI_BENCH_BURST_PER=100000

# Set a tighter warn threshold to make the warn-path fire earlier:
./gradlew :omniscience2:ingestBench \
    -DOMNI_BENCH_V2_CAP=1000
```

Available `OMNI_BENCH_*` overrides:

| Property | Default | Meaning |
|---|---|---|
| `OMNI_BENCH_SUSTAINED_RATE` | 10 000 | Open-loop producer rate for scenario 1. |
| `OMNI_BENCH_SUSTAINED_SEC` | 15 | Duration of scenario 1. |
| `OMNI_BENCH_BURST_THREADS` | 8 | Producer thread count for scenario 2. |
| `OMNI_BENCH_BURST_PER` | 25 000 | Records per producer thread in scenario 2. |
| `OMNI_BENCH_OVERLOAD` | 200 000 | Total records offered in scenario 3. |
| `OMNI_BENCH_V2_CAP` | 10 000 | v2 warn threshold for the bench (production default: 100 000). Env key retains the legacy `_CAP` suffix for CI back-compat; semantic is warn threshold, not drop ceiling. |

The bench writes JSON to `omniscience2/build/reports/ingest-bench.json` for trend tracking across runs.
