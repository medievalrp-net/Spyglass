# Omniscience2 — gap report

Source-code comparison between v1 (`../Omniscience`) and v2 (this repo). Written after reading every Java file in both repos plus the test suites and analysis docs. The intent is to catalog every concrete difference so downstream work can be planned against a stable reference.
...
| Status | Files | Notes |
|--------|-------|-------|
| `D` | 8 [`plugin/migration/*.java`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/migration/) sources | `MigrationCommand`, `MigrationService`, `V1ToV2Translator`, `V1DocumentReader`, `V1ItemDecoder`, `V1Schema`, `WorldNameLookup`, `MigrationProgressStore` |
| `D` | 2 migration tests | `MigrationIT.java`, `V1ToV2TranslatorTest.java` |
| `??` | [`api/event/BlockUseRecord.java`](../../../api/src/main/java/net/medievalrp/omniscience2/api/event/BlockUseRecord.java) | New record type, untracked |
| `M` | ~30 files | The refactor waves themselves, not yet in HEAD |

**Implications:**
- The README still says `omniv2 admin migrate-v1` works ([`README.md:33`](../../../README.md)). It does not in the current working tree — acceptable per operator decision, but the README needs to match reality.
- `omniscience2.admin` permission is absent from [`plugin.yml`](../../../plugin/src/main/resources/plugin.yml).
...
| Threading | Sync `EntryQueueRunner` polled every 20 ticks via `BukkitScheduler` | Virtual-thread `omni2-drain` loop with batch drain (up to 512 per batch) |
...
| Lifecycle | Five singletons (`Omniscience.INSTANCE`, `OmniConfig.INSTANCE`, `OmniApi.omniscience`, `OmniEventRegistrar.INSTANCE`, `Omniscience.PLUGIN_INSTANCE`) | DI-style: every collaborator constructed in `Omniscience2Plugin.onEnable` and passed by reference |
| Public surface | API and core both under `net.medievalrp.omniscience.api.*` (boundary leak — concrete `Parameter*`/`Flag*` live in Core but in the `api` package) | Strict split: `net.medievalrp.omniscience2.api` vs `net.medievalrp.omniscience2.plugin`; everything plugin-side is `@ApiStatus.Internal` |
| API discovery | Static `OmniApi.getOmniscience()` | `Bukkit.getServicesManager().load(Omniscience2Api.class)` |
| Backends | Mongo + a stub `DynamoStorageHandler` (writes are no-ops, queries return null) | Mongo only |
| Command framework | Cloud, hand-built for each subcommand | Cloud, factored through services and a single `OmniCommands.register` |

---

## 3. API surface comparison

### v1 OmniscienceAPI (18 files)

```
api/                       OmniApi (static delegates)
...
api/interfaces/            IOmniscience (30-method grab-bag), WorldEditHandler
...
|---|---|---|
| `OmniApi` static singleton | `Bukkit.getServicesManager().load(Omniscience2Api.class)` | ✓ Improved |
...
| `EntryQueueRunner` (`runTaskTimerAsynchronously` every 20 ticks) | Virtual-thread `omni2-drain` loop with `poll(250ms)` | ✓ Better |
...
| `FlagHandler` interface + 8 concrete `Flag*` classes | `Flag` enum (5 values) + parsing inline in [`QueryStringParser:53-99`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/command/param/QueryStringParser.java) | ◐ Less extensible |
...
| `IOmniscience` (30 methods) | `Omniscience2Api` (8 methods) | ✓ Narrowed |
...
| `DataHelper`, `Formatter`, `InventoryUtil`, `PastTenseWithEnabled`, `TypeUtil`, `ReflectionHandler` | Pushed into plugin module ([`util/*`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/util/)) where they belong | ✓ Boundary fix |
...
- **v2's `EventRecord` is a sealed interface that permits only 16 specific records** ([`EventRecord.java:7-23`](../../../api/src/main/java/net/medievalrp/omniscience2/api/event/EventRecord.java)). External plugins that want to register their own custom event records **cannot** — the seal makes the type closed. v1's `OEntry.create().source(p).custom(eventName, wrapper).save()` API let consumers register arbitrary string event names. That extension point is gone, and there's no replacement. If a downstream MedievalRP plugin (Reserv, Cauldron, WhisperNet, VestaPersona, etc.) wants to write its own audit records into the same store, it has no v2 API path.
- **`registerDisplayRenderer(eventName, renderer)`** ([`Omniscience2Api.java:25`](../../../api/src/main/java/net/medievalrp/omniscience2/api/Omniscience2Api.java)) is stored in `Omniscience2ApiImpl` but never read back in [`ResultRenderer.java`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/command/render/ResultRenderer.java). The whole per-event display customization contract is half-built.
...
| | v1 | v2 |
|---|---|---|
| Plugin entry | `Omniscience.java` → `OmniCore` (access-modifier split) | [`Omniscience2Plugin.java`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/Omniscience2Plugin.java) (single class) |
| Config | `OmniConfig.INSTANCE.setup(FileConfiguration)` — 40 fields, silently defaults misspellings | `Omniscience2Config.load(plugin)` — HOCON, record-shaped, with auto-merge of new event keys |
...
| Listener registration | Two paths: `OmniCore.registerEventHandlers` + `OmniEventRegistrar.enableEvents` (per-event from config) | Single pass over `List<RecordingListener>`; a listener only registers if at least one of its `events()` is enabled |
...
**v1** ([`MongoStorageHandler.java`](../../../../Omniscience/Omniscience-Core/src/main/java/net/medievalrp/omniscience/io/mongo/MongoStorageHandler.java)) created two indexes plus TTL:
...
**v2** ([`IndexManager.java:13-25`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/storage/IndexManager.java)) creates four:
...
**However,** [`MongoRecordStore.query()`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/storage/MongoRecordStore.java) has a subtler issue. When the query has no `event` predicate, `candidateTypes` returns every distinct record class — and v2 then runs **one Mongo query per record type**, merges + re-sorts in Java, then truncates to `limit`. With 13 distinct record classes in [`EventCatalog`](../../../api/src/main/java/net/medievalrp/omniscience2/api/event/EventCatalog.java), an unfiltered search hits Mongo 13 times and returns up to `13 × limit` rows before truncating. For `limit=1000`, that's potentially 13k rows pulled to render 1k. v1's single `aggregate` pipeline was less prone to this.

Other storage notes:
- [`PredicateToBson.java`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/storage/PredicateToBson.java) is a clean ~50-line translator covering the entire predicate sealed hierarchy. ✓
- [`RollbackEffectCodec.java`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/storage/RollbackEffectCodec.java) is a polymorphic discriminator codec for the sealed `RollbackEffect` with a back-compat fallback for documents written before the discriminator existed. Non-trivial Mongo glue done right.
- [`UndoStack.java`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/rollback/UndoStack.java) persists undo history to Mongo with a 24h TTL index. v1's `lastActionResults` was a `Map<UUID, List<ActionResult>>` in `OmniCore` that died on every restart.
...
| `EventBundleListener` | `bundle-insert/extract` | `BundleTransactionListener` | ◐ Better diff via `BundleMeta` snapshot, but contains **debug logging left in production** ([`:70-85`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/listener/modern/BundleTransactionListener.java)) |
...
v1's [`EventBreakListener.saveDependantBreaks`](../../../../Omniscience/Omniscience-Core/src/main/java/net/medievalrp/omniscience/listener/block/EventBreakListener.java) tracks a `DependantStyle` taxonomy: `WALL` (wall banners, wall torches, cocoa, ladders, wall signs, tripwire hooks), `BOTTOM` (poppies, wheat, torches, redstone dust, carpets, rails, pressure plates, stems, signs), `TALL` (doors, tall grass, large fern, sunflowers, peonies), `ALL` (levers, buttons). A break records companion breaks for every dependent block — a torch on the side of a wall gets its own break record when the wall is destroyed.

v2's [`MultiBlockBreakListener`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/listener/block/MultiBlockBreakListener.java) only handles **bed + door + tall plant pairs**. A rollback of a wall break in v2 will leave torches, levers, pressure plates, signs, ladders, rails, carpets, and pressure plates floating in air — they'll either vanish from physics or stay as orphans with no break record.

### WorldEdit / FAWE

| | v1 | v2 |
|---|---|---|
| Vanilla WE subscriber | `WorldEditLogger.java` | [`WorldEditSubscriber.java`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/worldedit/WorldEditSubscriber.java) | ✓ |
| FAWE batch processor | `fawe/FaweBatchLogger.java` | [`worldedit/FaweBatchLogger.java`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/worldedit/FaweBatchLogger.java) | ✓ |
...
| `-we` flag | `FlagWorldEditSel.java` (has a bug where `isIgnoredDefault` return value is discarded) | [`QueryStringParser.java:70-86`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/command/param/QueryStringParser.java) + [`WorldEditSelection.java`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/worldedit/WorldEditSelection.java) | ✓ Fixed |

### Commands

v1 `/omniscience` (aliases `/o`, `/omni`) subcommands: `search`, `rollback`, `restore`, `page`, `undo`, `tool`, `events`, `ai`. Plus utility command `/omnitele`.

v2 `/omni2` (aliases `/omniv2`, `/o2`, `/omniscience2`) subcommands: `search`, `rollback`, `restore`, `page`, `undo`, `tool`, `events`, `help`.
...
| `/omnitele` | ✓ utility for clickable teleport from results | — | **Lost — v2 search results don't have clickable teleport**. v2's click event runs another search instead. |
...
| `-g` | `-g`/`-global` | ✓ |
| `-ng` | `-ng`/`-nogroup` | ✓ |
| `-ex` (FlagExtended) | `-ex`/`-extended` | ◐ **v2 parses `Flag.EXTENDED` but nothing reads it** — the v1 behavior (per-result inline location line) isn't implemented in [`ResultRenderer.java`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/command/render/ResultRenderer.java). Dead flag. |
...
| `-we` | `-we`/`-worldedit` | ✓ — but checks `omniscience2.worldedit` permission **which isn't declared in [`plugin.yml`](../../../plugin/src/main/resources/plugin.yml)**. Will always fail for non-ops. |
...
- **Clickable teleport from search results.** v1's [`SearchCallback.buildComponent`](../../../../Omniscience/Omniscience-Core/src/main/java/net/medievalrp/omniscience/command/async/SearchCallback.java) wires `ClickEvent.Action.RUN_COMMAND` to `/omnitele <world> <x> <y> <z>`, so clicking a search result teleports the operator to where the event happened. v2's [`ResultRenderer.line`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/command/render/ResultRenderer.java) runs another `/omni2 search` instead. No way to jump to a location from results.

### API extension points that weren't replaced

- **Custom event registration** — v1's `OEntry.create().source(p).custom(eventName, wrapper)` is gone. No v2 equivalent. Sister plugins (Reserv, Cauldron, WhisperNet, VestaPersona) that used this path have no way to register audit events against Omniscience2.
...
| Issue | File | Note |
|---|---|---|
| `SculkListener` hooks the wrong event | [`SculkListener.java`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/listener/modern/SculkListener.java) | Hooks `SculkBloomEvent` (catalyst growth on death) instead of sensor/shrieker activation by player. Semantic regression. |
| `crafter` vs `craft` event name | [`CrafterListener.java`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/listener/modern/CrafterListener.java), [`EventCatalog.java:64`](../../../api/src/main/java/net/medievalrp/omniscience2/api/event/EventCatalog.java) | v1 used `"craft"`, v2 uses `"crafter"`. Inconsistency only matters if cross-version querying is wanted. |
| `omniscience2.worldedit` permission undeclared | [`plugin.yml`](../../../plugin/src/main/resources/plugin.yml), checked in [`QueryStringParser:74`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/command/param/QueryStringParser.java) | Bukkit treats undeclared perms as default false. `-we` flag always fails for non-ops. |
| Bundle debug logging in production | [`BundleTransactionListener:70-85`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/listener/modern/BundleTransactionListener.java) | `plugin.getLogger().info("bundle-click: ...")` spams logs on every inventory click |
| `Flag.DRAIN` is dead | [`Flag.java`](../../../api/src/main/java/net/medievalrp/omniscience2/api/query/Flag.java), nowhere else | Defined in enum, never parsed, never consumed |
| `Flag.EXTENDED` is dead (behavior) | parsed in [`QueryStringParser:69`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/command/param/QueryStringParser.java), never read | v1's per-result inline location line isn't in v2's renderer |
| `Flag.GLOBAL` barely useful | parsed in [`QueryStringParser:65-66`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/command/param/QueryStringParser.java) | Used only as a local bool to suppress default radius; the enum value itself never read |
| `Messages.java` unused | [`command/render/Messages.java`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/command/render/Messages.java) | Full MiniMessage loader with `messages.conf` resolution, never instantiated anywhere. PLAN.md Block 12a (MiniMessage extraction) was prepared but not completed. |
| `registerDisplayRenderer` half-built | [`Omniscience2Api.java:25`](../../../api/src/main/java/net/medievalrp/omniscience2/api/Omniscience2Api.java), [`Omniscience2ApiImpl.java:65-67`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/api/Omniscience2ApiImpl.java) | Public API accepts registrations but `ResultRenderer` never looks them up |
| `MongoRecordStore.query` per-type fan-out | [`MongoRecordStore.java:97-108`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/storage/MongoRecordStore.java) | Up to 13 Mongo queries for an unfiltered search |
| `isWorldEditInstalled` doesn't check enabled state | [`Omniscience2Plugin:233`](../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/Omniscience2Plugin.java) | Edge case: another plugin disabling WE on startup |
| `Source` record is unsealed with 7 always-null fields | [`Source.java`](../../../api/src/main/java/net/medievalrp/omniscience2/api/event/Source.java) | Storage cost small; type is begging to be a sealed interface `Player | Entity | Plugin | Console | CommandBlock | Environment` |
...
| Mongo IT | — | [`MongoRecordStoreIT`](../../../plugin/src/test/java/net/medievalrp/omniscience2/plugin/storage/MongoRecordStoreIT.java) (Testcontainers, exercises round-trip + index existence + limit + item-field queries) |
| Service-layer tests | — | 8 files under [`command/service/`](../../../plugin/src/test/java/net/medievalrp/omniscience2/plugin/command/service/) |
| Pipeline tests | — | [`AsyncRecorderTest`](../../../plugin/src/test/java/net/medievalrp/omniscience2/plugin/pipeline/AsyncRecorderTest.java) covers batching, drop-on-full, flush-on-shutdown |
| Predicate tests | — | [`PredicateToBsonTest`](../../../plugin/src/test/java/net/medievalrp/omniscience2/plugin/storage/PredicateToBsonTest.java) covers the full sealed hierarchy |
...
```
plugin/
├── Omniscience2Plugin.java              bootstrap, DI wiring, listener registration, WE hookup
├── api/
│   └── Omniscience2ApiImpl.java         Omniscience2Api impl, Mongo-backed
├── command/
│   ├── OmniCommands.java                Cloud command registration (root + 8 subcommands × 4 aliases)
│   ├── OmniSuggestions.java             tab completion
...
├── config/
│   └── Omniscience2Config.java          HOCON config loader with auto-merge
├── listener/
