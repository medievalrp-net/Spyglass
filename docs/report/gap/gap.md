...
| Lifecycle | Five singletons (`v1.INSTANCE`, `OmniConfig.INSTANCE`, `OmniApi.v1`, `OmniEventRegistrar.INSTANCE`, `v1.PLUGIN_INSTANCE`) | DI-style: every collaborator constructed in `SpyglassPlugin.onEnable` and passed by reference |
...
| API discovery | Static `OmniApi.getv1()` | `Bukkit.getServicesManager().load(SpyglassApi.class)` |
...
|---|---|---|
| `OmniApi` static singleton | `Bukkit.getServicesManager().load(SpyglassApi.class)` | ✓ Improved |
...
| | v1 | v2 |
|---|---|---|
| Plugin entry | `v1.java` → `OmniCore` (access-modifier split) | [`SpyglassPlugin.java`](../../../plugin/src/main/java/net/medievalrp/spyglass/plugin/SpyglassPlugin.java) (single class) |
| Config | `OmniConfig.INSTANCE.setup(FileConfiguration)` — 40 fields, silently defaults misspellings | `SpyglassConfig.load(plugin)` — HOCON, record-shaped, with auto-merge of new event keys |
...
- [`UndoStack.java`](../../../plugin/src/main/java/net/medievalrp/spyglass/plugin/rollback/UndoStack.java) persists undo history to Mongo with a 24h TTL index. v1's `lastActionResults` was a `Map<UUID, List<ActionResult>>` in `OmniCore` that died on every restart.
...
v1 `/v1` (aliases `/o`, `/omni`) subcommands: `search`, `rollback`, `restore`, `page`, `undo`, `tool`, `events`, `ai`. Plus utility command `/omnitele`.

v2 `/omni2` (aliases `/omniv2`, `/o2`, `/spyglass`) subcommands: `search`, `rollback`, `restore`, `page`, `undo`, `tool`, `events`, `help`.
...
- **Clickable teleport from search results.** v1's [`SearchCallback.buildComponent`](../../../../v1/the v1 core/src/main/java/net/medievalrp/v1/command/async/SearchCallback.java) wires `ClickEvent.Action.RUN_COMMAND` to `/omnitele <world> <x> <y> <z>`, so clicking a search result teleports the operator to where the event happened. v2's [`ResultRenderer.line`](../../../plugin/src/main/java/net/medievalrp/spyglass/plugin/command/render/ResultRenderer.java) runs another `/omni2 search` instead. No way to jump to a location from results.
