...
The signal is: a sister plugin files a concrete ask ("we want to write X event, queryable by field Y, rendered alongside sg search results"). At that point, evaluate whether the ask fits an existing record type (preferred) or needs an escape hatch. Until that ask arrives, the seal stays.
