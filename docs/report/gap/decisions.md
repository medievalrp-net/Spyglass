...
v1's public API included `OEntry.create().source(p).custom(eventName, wrapperData)` — any downstream plugin could mint event names on the fly and hand a `DataWrapper` full of arbitrary fields to Omniscience's storage. The records rode the same collection as first-party events and were queryable through the same parameter handlers.

v2's [`EventRecord`](../../../api/src/main/java/net/medievalrp/omniscience2/api/event/EventRecord.java) is a sealed interface. Downstream plugins that depend on `omniscience2-api` cannot add a new `permits` entry — sealed types are a closed set. `Omniscience2Api.record(EventRecord)` only accepts the 17 first-party records.
...
**(b) Add an escape hatch.** Introduce `CustomEventRecord(..., String customEventName, Map<String, Object> payload)` as another permit, so downstream plugins can construct one and hand it to `Omniscience2Api.record()`.
...
4. **If a sister plugin needs audit logging, its own collection is cleaner.** The plugin owns its schema, picks its own indexes, writes its own query command. It loses Omniscience2's timeline alongside its own records, but that's a genuinely separate concern — a plugin's internal audit log doesn't have to share rendering or rollback with Omniscience's world-events log.
