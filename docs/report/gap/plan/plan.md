...
- **Priority:** P0
- **Effort:** S
- **Acceptance:** a non-op player with `spyglass.worldedit` granted via LuckPerms can use `/omni2 search a:break -we`.
...
- **How:**
  - Add a `/omni2 tele <worldUuid> <x> <y> <z>` subcommand in [`OmniCommands`](../../../../plugin/src/main/java/net/medievalrp/spyglass/plugin/command/OmniCommands.java), gated on `spyglass.tele` permission (new, default op).
  - In [`ResultRenderer.line`](../../../../plugin/src/main/java/net/medievalrp/spyglass/plugin/command/render/ResultRenderer.java), change the click event to `ClickEvent.runCommand("/omni2 tele " + loc.worldId() + " " + loc.x() + " " + loc.y() + " " + loc.z())`.
