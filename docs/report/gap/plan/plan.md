# Omniscience2 — gap-fix execution plan

Companion to [`../gap.md`](../gap.md). Every entry references the gap-report section it addresses so nothing is fixed in a vacuum.

**Scoping decision:** the v1 → v2 migration tool is **not** being restored. All gaps related to migration (deleted `plugin/migration/` sources, `omniscience2.admin` permission, `admin migrate-v1` command, README upgrade instructions) are handled in Phase 0 as a cleanup-and-document task, not a restoration.
...
- **What:** `plugin/src/main/java/net/medievalrp/omniscience2/plugin/migration/` and `plugin/src/test/java/net/medievalrp/omniscience2/plugin/migration/` are marked `D` on disk. Per operator decision, migration is not being restored.
- **How:**
  ```bash
  git rm plugin/src/main/java/net/medievalrp/omniscience2/plugin/migration/*.java
  git rm plugin/src/test/java/net/medievalrp/omniscience2/plugin/migration/*.java
  ```
...
- **What:**
  - [`README.md:26-38`](../../../../README.md) — "Upgrading from v1" section describing `omniv2 admin migrate-v1`
  - [`README.md:72-74`](../../../../README.md) — "Wire-format compatibility" section that only exists to justify migration
  - `plugin.yml` — add/remove `omniscience2.admin` (see below)
- **How:**
  - Delete the "Upgrading from v1" section entirely. Replace with a single line: *This plugin is a fresh install with no migration path from the legacy MPL Omniscience. See the `Omniscience/` sibling repo for historical reference only.*
  - Delete the "Wire-format compatibility" section — with migration gone, the schema can evolve freely.
- **Priority:** P0
- **Effort:** S
- **Acceptance:** README no longer mentions `migrate-v1`, `Omniscience.DataEntry`, or v1-compat schema claims.
...
### 1.2 Declare `omniscience2.worldedit` permission

- **What:** [`QueryStringParser:74`](../../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/command/param/QueryStringParser.java) gates the `-we` flag behind a permission that isn't declared in [`plugin.yml`](../../../../plugin/src/main/resources/plugin.yml). Bukkit defaults undeclared perms to false for non-ops.
- **How:** add to `plugin.yml`:
  ```yaml
  omniscience2.worldedit:
    default: op
  ```
- **Priority:** P0
- **Effort:** S
- **Acceptance:** a non-op player with `omniscience2.worldedit` granted via LuckPerms can use `/omni2 search a:break -we`.

### 1.3 Remove Bundle debug logging

- **What:** [`BundleTransactionListener:70-85`](../../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/listener/modern/BundleTransactionListener.java) logs `bundle-click: ...` and `bundle-diff: ...` on every relevant click.
...
- **What:** v1 emitted `craft`; v2 emits `crafter` from [`CrafterListener`](../../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/listener/modern/CrafterListener.java). The [`EventCatalog`](../../../../api/src/main/java/net/medievalrp/omniscience2/api/event/EventCatalog.java) and [`config.conf`](../../../../plugin/src/main/resources/config.conf) use `crafter`. Trivial inconsistency, pick one.
...
- **How:**
  ...
  - `Flag.EXTENDED` — either:
    ...
    - **Recommendation:** (a). The extended view is genuinely useful for staff investigations. Low effort given [`ResultRenderer.hover`](../../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/command/render/ResultRenderer.java) already knows how to format a location.
...
- **What:** [`Messages.java`](../../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/command/render/Messages.java) is a full MiniMessage template loader with placeholder support, never called anywhere.
- **How:** decide:
...
- **What:** [`Omniscience2Api.registerDisplayRenderer`](../../../../api/src/main/java/net/medievalrp/omniscience2/api/Omniscience2Api.java) accepts renderer registrations via `Omniscience2ApiImpl.renderers` but [`ResultRenderer`](../../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/command/render/ResultRenderer.java) never looks them up.
- **How:** decide:
  - (a) **Wire it in.** Inject `Omniscience2Api` into `ResultRenderer`. In `line`, check `api.displayRenderer(record.event())` — if present, call its `renderTarget(record, defaultTarget, flags)` instead of the default, and append its `hoverLines(record)` to the hover.
  - (b) **Remove from API.** If extension isn't on the roadmap, cut the method from the interface and drop `renderers` from the impl.
  - **Recommendation:** (a). The extension point is near-free to honor and enables sister plugins (Reserv, Cauldron, etc.) to add event-specific chat polish without patching Omniscience2.
- **Priority:** P2
- **Effort:** M
...
- **What:** v1's [`EventCommandListener.onServerCommand`](../../../../../Omniscience/Omniscience-Core/src/main/java/net/medievalrp/omniscience/listener/chat/EventCommandListener.java) logged commands typed at the console. v2's [`CommandListener`](../../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/listener/chat/CommandListener.java) only hooks `PlayerCommandPreprocessEvent`.
...
- **What:** v1 results had a `ClickEvent.Action.RUN_COMMAND → /omnitele <world> <x> <y> <z>`, jumping the operator to where the event happened. v2 runs another `/omni2 search` instead.
- **How:**
  - Add a `/omni2 tele <worldUuid> <x> <y> <z>` subcommand in [`OmniCommands`](../../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/command/OmniCommands.java), gated on `omniscience2.tele` permission (new, default op).
  - In [`ResultRenderer.line`](../../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/command/render/ResultRenderer.java), change the click event to `ClickEvent.runCommand("/omni2 tele " + loc.worldId() + " " + loc.x() + " " + loc.y() + " " + loc.z())`.
...
- **How:** new `MessageParam` under [`plugin/command/param/`](../../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/command/param/). Aliases `m`, `message`. Parse behavior: regex substring search against the `message` field on `ChatRecord` (and `commandLine` on `CommandRecord` — need Or-across-paths like `ItemFieldParams`). Register in [`Omniscience2Plugin.onEnable`](../../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/Omniscience2Plugin.java) param list.
- **Priority:** P1
- **Effort:** M
- **Acceptance:** `/omni2 search a:say,command m:diamond` returns chats and commands mentioning diamond.
...
- **Priority:** P1
- **Effort:** M
- **Acceptance:** `/omni2 search a:break c:creeper t:1d -g` returns creeper-caused breaks.
...
- **Priority:** P1
- **Effort:** S each
- **Acceptance:** `/omni2 search i:diamond_sword`, `/omni2 search trg:chest` both work.
...
- **Priority:** P2
- **Effort:** S
- **Acceptance:** `/omni2 search a:join ip:192.168.1.100 t:30d` finds historical joins from that IP.
...
- **Priority:** P2
- **Effort:** S
- **Acceptance:** `/omni2 search a:say rcp:Alice t:1d` returns chats where Alice was a recipient.
...
- **What:** v1 logged `close` for any `Container`/`DoubleChest`. v2's [`ContainerInteractListener.onInventoryClose`](../../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/listener/container/ContainerInteractListener.java) only fires for `ShulkerBox`.
...
- **How:** add `BlockDispenseEvent` handler to existing [`ItemDropListener`](../../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/listener/item/ItemDropListener.java). Source is the dispenser block; Origin is environment.
- **Priority:** P2
- **Effort:** S
...
- **What:** [`MongoRecordStore.query()`](../../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/storage/MongoRecordStore.java) runs up to 13 separate Mongo queries when there's no `event` predicate, merges results in Java, re-sorts, and truncates.
...
- **Acceptance:** unfiltered `/omni2 search t:1h -g` performs one Mongo query instead of 13; `MongoRecordStoreIT` still passes.

### 5.2 Make `Source` a sealed interface

- **What:** [`Source`](../../../../api/src/main/java/net/medievalrp/omniscience2/api/event/Source.java) is a flat record with 8 fields, most null per kind. Storage is slightly wasteful, and the type screams for sealed hierarchy.
- **How:**
  ```java
...
- **What:** [`Omniscience2Plugin:233`](../../../../plugin/src/main/java/net/medievalrp/omniscience2/plugin/Omniscience2Plugin.java) checks plugin presence, not enabled state.
...
- Add a case to [`regression/cases.json`](../../../../regression/cases.json) for each restored listener:
  - server command (`/omni2 search a:command -g`)
...
| 1.2 | P0 | S | Declare `omniscience2.worldedit` perm |
