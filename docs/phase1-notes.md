...
### API module (`api/`)
- `Omniscience2Api` service interface (registered via Bukkit `ServicesManager`).
...
- `Omniscience2Plugin` main class, wires everything via constructor injection.
...
Phase 1 was verified against `../RP_Server` (Paper 1.21.8) running alongside the original MPL-licensed `Omniscience.jar`. Both plugins coexisted cleanly (separate classloaders, separate Mongo databases, separate commands). Steps actually executed:

1. `./gradlew test` — 12 tests green (API `DurationTest` + plugin `PredicateToBsonTest` + plugin `AsyncRecorderTest`).
2. `./gradlew :plugin:shadowJar` — produced `plugin/build/libs/Omniscience2-0.1.0-SNAPSHOT.jar`.
3. Jar copied to `../RP_Server/plugins/Omniscience2.jar`.
4. Server launched; Omniscience2 enabled, indexes `{source.playerId, occurred}`, `{event, occurred}`, `{location.worldId, location.x, location.z, location.y, occurred}`, TTL on `expiresAt` confirmed via direct Mongo inspection (`pymongo`).
5. Seeded 6 synthetic records (3 break, 1 place, 1 say, 1 join) directly into `Omniscience2.EventRecords` via `pymongo`.
...
7. Regression of original Omniscience v1:
