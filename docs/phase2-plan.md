...
- `/sg ai <question>` command; extract `/sg ...` suggestions from the response and render them as clickable chips with `ClickEvent.suggestCommand(...)`.

### B3. Inspection wand (`/sg tool`)
...
### C5. Migration tool
- Read v1's `DataEntry` collection from `v1` database, translate each document to a v2 record, write to `EventRecords` in `Spyglass` database.
- Implement as a standalone gradle task or a `/sg migrate-v1` admin command (gated behind a confirmation prompt).
