...
`/sg reload` (permission `spyglass.reload`, default op) validates `config.conf` before applying event enabled/past-tense/retention settings, global event retention, command redaction, and wand material/lookback. Existing tagged wands remain protected when disabled or after changing material. Inspection still requires `spyglass.tool`. Wands cannot be placed, renamed, consumed in processing inventories, dispensed, or dropped into the world. Death/container drops discard tagged wands; `/sg tool` reissues one. A full inventory must have space before a new wand is issued.
