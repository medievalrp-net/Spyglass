# v1 Dissection — 05: Commands and Display Handlers

Files covered:

- Core: `command/SpyglassCommands.java`, `command/AiHandler.java`, `command/PageStore.java`,
...
This is the user-facing layer: how a player goes from typing `/sg s p:Steve b:stone` to
...
The whole surface was migrated from a bespoke `sg`/`SimpleCommand` hierarchy
to Incendo Cloud 2.x in this session. The migration was mostly mechanical — the command
handlers moved into `SpyglassCommands.register` but the rendering logic (`SearchCallback`,
...
All subcommands registered in `SpyglassCommands.register` (`SpyglassCommands.java:76-137`).
Top-level command is registered with three aliases at `SpyglassCommands.java:83`:
`/v1`, `/o`, `/sg`. Every subcommand below is reachable through all three
root aliases.
...
| `search` | `s`, `sc`, `lookup`, `l` | `v1.commands.search` | greedy string | `runSearch` |
| `page` | `p`, `pg` | `v1.commands.page` | int ≥1 | `PageStore.showPage` |
| `rollback` | `rb`, `roll` | `v1.commands.rollback` | greedy string | `runApplier(NEWEST_FIRST)` |
| `restore` | `rs`, `rst` | `v1.commands.rollback` | greedy string | `runApplier(OLDEST_FIRST)` |
| `undo` | `u` | `v1.commands.undo` | — | `runUndo` |
| `tool` | `t`, `inspect` | `v1.commands.tool` | — | `runTool` |
| `events` | `e` | `v1.commands.events` | — | `runEvents` |
| `ai` | `help-ai`, `ask` | `v1.commands.ai` | greedy string | `aiHandler.execute` |

Plus a second top-level command `/sgtele` (`SpyglassCommands.java:127-134`). Signature is
`/sgtele <worldUuid> <x> <y> <z>`. Permission is `v1.mayuse`. It is *not*
...
```
/sgtele <worldUuid> <blockX> <blockY> <blockZ>
```

There's no token, no nonce, no expiration. Anyone with `v1.mayuse` can teleport
...
The help text at `SpyglassCommands.java:56-65` is a `List<HelpEntry>` that only lists the
canonical names (no aliases displayed). Aliases are invisible to users who don't already
know them.

## Cloud integration

Setup at `SpyglassCommands.java:76-80`:
...
direct invoker (console or player). `SpyglassCommands.sender(...)` at
`SpyglassCommands.java:150-157` unwraps this:
...
grabs everything after the last space (`SpyglassCommands.java:159-163`). That single token
...
The old command code (pre-this-session) was a tree of `sg` abstract
...
## The main flow: `/sg search …`

```
/sg search p:Steve b:stone
    │
    ▼
Cloud dispatch → runSearch(sender, "p:Steve b:stone") SpyglassCommands.java:177
    │
    ▼
...
Async.lookup(session, new SearchCallback(session)) Async.java:16
    │
    ├─ query.setSearchLimit(sg.lookupSizeLimit)
...
public static void lookup(final QuerySession session, AsyncCallback callback) {
    session.getQuery().setSearchLimit(sg.INSTANCE.getLookupSizeLimit());
    Bukkit.getScheduler().runTaskAsynchronously(v1.getProvidingPlugin(v1.class), () -> {
        try {
            CompletableFuture<List<DataEntry>> future = v1.getStorageHandler().records().query(session);
...
pattern applies in `runApplier` at `SpyglassCommands.java:206` with
`limits.actionables`.
...
if (oDHandler.isPresent()) {
    displayHandler = v1.getDisplayHandler(oDHandler.get());
    ...
}
```
...
results). The click re-runs a constructed `/v1 search` with narrowed
...
private String buildDetailCommand(DataEntry entry) {
    ...
    StringBuilder command = new StringBuilder("/v1 search ");
    command.append(action).append(" ").append(source).append(" ").append(target).append(" ");
    Optional<ParameterHandler> radius = v1.getParameterHandler("r");
...
Hard-codes the prefix `/v1` — that is, the click won't land if only `/sg` or
...
`/sg rollback <params>` and `/sg restore <params>` both route to
`runApplier(sender, paramString, sort)` (`SpyglassCommands.java:193-271`). Logic:
...
6. For player senders, stash results in `v1.addLastActionResults(uuid, list)`
   for `/sg undo`.
...
`/sg undo` pulls the last list of `ActionResult`s for a player, reverses it, and
walks each transaction (`SpyglassCommands.java:273-333`). The implementation is
...
Also: `lastActionResults` is in-memory (stored in `sg.lastActionResults`). Server
restart eats it. Mentioned in doc 01 as a known limitation.

## Tool toggle: `runTool`

`/sg tool` (`SpyglassCommands.java:335-374`) toggles the data wand for a player. State
lives in `sg.activeWandList` (a `Set<UUID>`). The interaction is convoluted —
...
Two lines (`SpyglassCommands.java:376-381`):

```java
List<String> enabledEvents = sg.getEnabledEvents();
enabledEvents.sort(String::compareToIgnoreCase);
sender.sendMessage(Formatter.bonus(String.join(", ", enabledEvents)));
```

Pulls the enabled set from `sg` (via `sg`), sorts, prints
...
`v1.mayuse` gates it at the command level. But see the pain points: the
...
`SpyglassCommands.register` call. It is invoked as `/sg ai <question>` and dispatches to
Vertex AI's Gemini `generateContent` endpoint, streams back the text, parses out any
`/sg ...` commands, and renders them as clickable chips in chat.

### Gate checks

Three gates before the request fires (`AiHandler.java:64-91`):

1. `sg.INSTANCE.isAIEnabled()` — config-level kill switch.
...
3. For each line, run it through `COMMAND_PATTERN` (`AiHandler.java:36`):
   ...
   private static final Pattern COMMAND_PATTERN =
       Pattern.compile("`(/sg[^`]+)`|(/sg\\s+\\S+(?:\\s+\\S+)*)");
   ```

4. If the line contains a backticked or bare `/sg ...` command, split it into
...
Set up in `sg.registerDisplayHandlers` (`sg.java:176-180`):
...
Dispatched by `sg.getDisplayHandler(key)` at `sg.java:192-194` which
...
- `buildTargetMessage`: if grouping is disabled (`Flag.NO_GROUP`) and the entry has a
  ...
  (default rendering uses the "Message" generic target). This is what makes `/sg s
...
```
/sgtele <worldUuid> <x> <y> <z>
```

Anyone with `v1.mayuse` (i.e., anyone who can use v1) can teleport
to arbitrary coordinates. The permission was meant to mean "can run v1
...
result hover" and "I'm running /sgtele" — they're functionally identical from the
server's perspective.

Options for v2:

- Tokenise: emit the click target as `/sgtele <ephemeralToken>` where the token is
  ...
  token."
- Permission node `v1.commands.teleport` distinct from `.mayuse`, and only
...
(`SpyglassCommands.java:90, 95, 100, 105, 110, 114, 118, 122`). Want to rename `page` to
...
### `sg.getDisplayHandler` is O(n)

Five registered handlers, four in use, so this doesn't matter in practice. But
`sg.java:192-194` streams the list and `filter`s on every call. In
...
4. **Virtual threads for `AiHandler`'s HTTP call.** Replace the double-nested
   `BukkitRunnable` with `Thread.ofVirtual().name("sg-ai-" + playerId).start(() ->
...
6. **Nonce-based `/sgtele`.** Emit click targets as `/sgtele <token>` where
...
7. **Cloud annotation-based command discovery.** Cloud 2.x supports
   ...
   `SpyglassCommands.register`. Aliases can be declared alongside: `@CommandMethod("sg|o|
   v1 search|s|sc|lookup|l <params>")`.
...
- **The detail-click drill-down** (`buildDetailCommand`) is a good UX — click a grouped
  ...
  the `v1` vs `sg` vs `o` alias choice without hard-coding.

- **AI help.** The `/sg ai` flow is genuinely useful. Tidy the threading, port the
