...
`/v1`, `/o`, `/omni`. Every subcommand below is reachable through all three
root aliases.
...
`/omnitele <worldUuid> <x> <y> <z>`. Permission is `v1.mayuse`. It is *not*
...
Hard-codes the prefix `/v1` — that is, the click won't land if only `/omni` or
...
- **The detail-click drill-down** (`buildDetailCommand`) is a good UX — click a grouped
  ...
  the `v1` vs `omni` vs `o` alias choice without hard-coding.
