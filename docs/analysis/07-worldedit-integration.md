# Omniscience Dissection — 07: WorldEdit and FastAsyncWorldEdit Integration

Files covered:

- Core: `Omniscience-Core/src/main/java/net/medievalrp/omniscience/worldedit/WorldEditLogger.java`
- Core: `Omniscience-Core/src/main/java/net/medievalrp/omniscience/worldedit/FAWERollbackHandler.java` (dead)
- Core: `Omniscience-Core/src/main/java/net/medievalrp/omniscience/worldedit/fawe/FaweBatchLogger.java`
- Core: `Omniscience-Core/src/main/java/net/medievalrp/omniscience/worldedit/fawe/FaweHook.java`
- Core: `Omniscience-Core/src/main/java/net/medievalrp/omniscience/worldedit/fawe/FaweTileCapture.java`
- Core (lives under an API-shaped package): `Omniscience-Core/src/main/java/net/medievalrp/omniscience/api/flag/FlagWorldEditSel.java`
- API: `OmniscienceAPI/src/main/java/net/medievalrp/omniscience/api/interfaces/WorldEditHandler.java`
- Glue: `OmniCore.onWorldEditStatusChange` (`OmniCore.java:159`), `PluginInteractionListener.handleToggle` (`PluginInteractionListener.java:22`), `SearchCallback.formatOriginTag` (`SearchCallback.java:168`), `OEntry.with` (`OEntry.java:48`), `OEntry.custom` (`OEntry.java:442`).

## Responsibility

Log a WorldEdit operation with the same fidelity as a hand-placed / hand-broken block: material, block data, container inventories, tile entity metadata, source player. Make those logs searchable (`a:break`, `p:Steve`, `r:30`, …) and rollback-able (`/omni rollback`) with no loss of information relative to the vanilla block listener path. Do this correctly on two backends:
...
- **`FlagWorldEditSel`** (`FlagWorldEditSel.java:25`) — the `-we` search flag. Adds a `SearchConditionGroup` matching the player's current WE cuboid selection (world + X/Y/Z ranges) onto the query. Registered conditionally by `OmniCore.onWorldEditStatusChange` only when WE is both loaded and enabled in config.

### Unused API

- **`WorldEditHandler`** (`WorldEditHandler.java:3`) — two-method interface (`enableWorldEditLogging`, `disableWorldEditLogging`) on the API surface. `OmniCore` has a slot for one (`OmniCore.java:38`) and a setter (`registerWorldEditHandler` at `OmniCore.java:322`), but nothing in-tree ever registers a handler and nothing ever calls either method on it. It exists so external plugins could theoretically register their own WE-logging backend; no one does.
...
**Registration.** `FlagWorldEditSel` is not registered unconditionally. `OmniCore.registerFlags()` (`OmniCore.java:144`) adds the stock flags and then checks `Bukkit.getPluginManager().isPluginEnabled("WorldEdit")` at `OmniCore.java:154`; if true, calls `onWorldEditStatusChange(true)` (`OmniCore.java:155`). `onWorldEditStatusChange` is at `OmniCore.java:159`, and it:
...
`OmniscienceAPI/.../interfaces/WorldEditHandler.java`:
...
`Omniscience-Core/build.gradle:13`:

```groovy
compileOnly files("$projectDir/libs/FastAsyncWorldEdit.jar")
```

FAWE is a `compileOnly` dependency (so it's not shaded into the plugin jar) and it's sourced from a local file dropped into `Omniscience-Core/libs/`. The `libs/` directory is gitignored (the file exists locally per `ls` but is not tracked), which means:
...
9. **`WorldEditHandler` is a phantom API surface.** Interface exists, field exists, setter exists, zero real uses. External plugins that see `OmniApi.registerWorldEditHandler(...)` might reasonably assume "if I register my own handler Omniscience will use it" — nothing downstream does. Either implement the contract (delegate to the registered handler from `onWorldEditStatusChange` when one is present) or delete the interface.
...
11. **Add test coverage.** Minimum:
    ...
    - Integration (requires a test server fixture): run a `//set stone` over a chest, query the resulting entries, run `/omni rollback`, verify the chest comes back with contents.
