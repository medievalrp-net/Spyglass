...
- **`RadiusParameter`** enforces `OmniConfig.INSTANCE.getRadiusLimit()` unless the sender has `v1.override.maxradius` (`RadiusParameter.java:42`). On limit breach it mutates the value, sends a warning, and proceeds — it does *not* reject the query.
...
Registered in `OmniCore.registerFlags()` (`OmniCore.java:144–157`). All flags live in `the v1 core/.../api/flag/` even though the API package prefix makes them look API-side.
