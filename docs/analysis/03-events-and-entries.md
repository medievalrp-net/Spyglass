# Omniscience Dissection — 03: Events and Entries

Files covered:

- **API entry model** (`OmniscienceAPI/src/main/java/net/medievalrp/omniscience/api/entry/`): `DataEntry.java`, `DataEntryComplete.java`, `DataAggregateEntry.java`, `OEntry.java`, `EntryQueue.java`, `ActionResult.java`, `Actionable.java`, `ActionableException.java`, `SkipReason.java`.
- **Core entry impls** (`Omniscience-Core/src/main/java/net/medievalrp/omniscience/api/entry/`): `BlockEntry.java`, `ContainerEntry.java`, `EntityEntry.java`, `EntryQueueRunner.java`.
- **Listener hierarchy** (`Omniscience-Core/src/main/java/net/medievalrp/omniscience/listener/`): 30 classes across `block/` (12), `item/` (7), `entity/` (4), `chat/` (2), `player/` (3), plus top-level `OmniListener`, `WandInteractListener`, `PluginInteractionListener`, `CraftBookSignListener`, `PermissionListener`.
...
There is also a secondary, mostly-orthogonal responsibility: **rollback/restore**. `DataEntry` subclasses in Core implement `Actionable` so the same record that was written during logging can be walked backward during `/omni rollback`. That path is covered in depth in `06-rollback.md`; this doc only notes the coupling.
...
These three all live under `net.medievalrp.omniscience.api.entry` despite being Core-internal — the API/impl boundary leak called out in `00-overview.md`.
...
- **`EntryQueueRunner`** (`EntryQueueRunner.java:10`) — `Runnable`. Drains the entire deque into a `List<DataWrapper>` every invocation, then hands the whole batch to `Omniscience.getStorageHandler().records().write(...)`. Scheduled by `OmniCore.onEnable` at 20-tick intervals asynchronously.

### Listeners

Every listener other than the top-level five extends **`OmniListener`** (`OmniListener.java:7`), which is a thin abstract `Listener` that holds an `ImmutableList<String>` of event names it handles and an `enabled` flag toggled by `OmniEventRegistrar.enableEvents`. A single `protected boolean isEnabled(String event)` method defers to the registrar singleton.
...
- **`OmniListener.java`** — the abstract base described above.
- **`WandInteractListener.java`** — Not an `OmniListener`. Implements `Listener` directly. Three handlers: `onPlayerJoin` auto-activates the wand for players with `omniscience.commands.search.autotool`; `onPlayerInteract` performs a location lookup when clicking with the wand material; `onBlockPlace` does the same for block-place interactions. Builds `QuerySession` + `SearchConditionGroup` in-line and hands to `Async.lookup`. This is not an event-logging listener — it's a query-entry-point listener.
- **`PluginInteractionListener.java`** — `PluginEnable/DisableEvent`. Toggles WorldEdit integration via `Omniscience.onWorldEditStatusChange`; the CraftBook branch is empty.
- **`CraftBookSignListener.java`** — single handler. Writes a `useSign` entry on any sign right-click. Only registered if `OmniConfig.doCraftBookInteraction()` + CraftBook is enabled.
...
**Step 2 — `EventBreakListener.onBlockBreak` fires** (`EventBreakListener.java:39`). The listener was registered back at `OmniCore.registerEventHandlers` → `OmniEventRegistrar.enableEvents` (`OmniEventRegistrar.java:172`) which only registered it because `config.yml` had `events.break.enabled: true` and `OmniConfig.setup` added the entry at `OmniConfig.java:89`.
...
**Step 6 — `.save()`** (`OEntry.java:53`). Validates `OmniApi.isEventRegistered(eventName)` — throws `IllegalArgumentException` otherwise. Sets `EVENT_NAME` and `CREATED` (current `Date`). Then walks another `instanceof` ladder on the source to decide whether to key the entry as `PLAYER_ID` (UUID string) or `CAUSE` (string like "environment", "console", "pl@SomePlugin", entity type, etc.). Finally: `EntryQueue.submit(wrapper)`.
...
**Step 10 — Much later, `/omni rollback p:alice 1h r:20`** queries for `BlockEntry` records (the event-class mapping — `"break"` → `BlockEntry.class` — comes from `OmniCore.registerEventWrapperClasses` at line 103). `DataEntry.from(eventName, false)` (`DataEntry.java:15`) does `OmniApi.getEventClass("break").orElse(DataEntryComplete.class).getConstructor().newInstance()`, then assigns `entry.data = <loaded wrapper>`. The `Actionable.rollback()` method on the reconstituted `BlockEntry` reverses step 5.

## Event-name registration: how the two registries connect

Omniscience has **two** parallel event-name tables and they are not the same thing. You cannot skip this if you want to understand how a custom event plugs in.

### Table A: name → past tense + enabled flag (`OmniEventRegistrar.eventMapping`)

Populated at config load time by `OmniConfig.setup` (`OmniConfig.java:87-90`):
```java
...
```
External plugins can add entries through `OmniApi.registerEvent(name, pastTense)` (`OmniApi.java:91`), which delegates to `OmniCore.registerEvent(String, String)` (`OmniCore.java:233`), which calls `addEvent(event, pastTense, enabled=true)`. No way to register as initially disabled — external events always enable themselves.

This table is consulted by:
- `OmniApi.isEventRegistered` (called from `OEntry.save` and `EntryQueue.submit` for validation)
- `OmniApi.getEventPastTense` (used in search-result rendering)
- `OmniEventRegistrar.enableEvents` (to decide which `OmniListener` instances to hook up with the `PluginManager`)

### Table B: name → `DataEntry` subclass (`OmniCore.eventMap`)

Populated by the hard-coded `registerEventWrapperClasses` (`OmniCore.java:103-111`):
```java
...
External plugins **cannot** add to Table B. The `IOmniscience.registerEvent(String, String)` signature only adds to Table A. The Class-typed overload `OmniCore.registerEvent(String, Class<? extends DataEntry>)` is package-private and there's no API passthrough. A consumer plugin wanting their custom event to be rolled back has no hook.

### How `OmniListener.handles` ties in

`OmniEventRegistrar.enableEvents` (`OmniEventRegistrar.java:172`) iterates `eventMapping` (Table A) and for each `(name, PastTenseWithEnabled)` pair finds the first `OmniListener` where `listener.handles(name)` returns true. If found AND enabled, `pm.registerEvents(listener, plugin)`. One listener can claim multiple names via its `ImmutableList<String>` ctor arg (see `EventHitListener` handling both `hit` and `shot`). No name can be claimed by two listeners — `findFirst` silently takes the first.

The mechanism is sound and pluggable enough that `config.yml` can genuinely flip events off. The catch is that the `OmniListener` instances themselves are hard-coded in `OmniEventRegistrar`'s constructor (`OmniEventRegistrar.java:29-76`) — external plugins can register arbitrary listeners with Bukkit directly, of course, but there's no API to plug an `OmniListener` into this registry. If you want an external event to respect config's `events.foo.enabled: false` flag, you have to re-implement the gate.

## Pain points

### 1. Listener boilerplate is copy-paste

The 30 `OmniListener` classes follow nearly the same shape: one or more `@EventHandler`, usually `ignoreCancelled = true, priority = MONITOR`, extract a handful of fields, call `OEntry.create().source(x).<verb>(...).save()`. The similarity is striking:
...
`BlockEntry.java`, `ContainerEntry.java`, `EntityEntry.java`, and `EntryQueueRunner.java` are under `net.medievalrp.omniscience.api.entry` in the **Core module**. The package root is the same as the API module's `net.medievalrp.omniscience.api.entry`. At runtime they coexist in one classloader because Core shades the API. An external plugin that imports OmniscienceAPI sees `DataEntry`, `DataEntryComplete`, `DataAggregateEntry`, `OEntry`, `EntryQueue`, `ActionResult`, `Actionable`, `ActionableException`, `SkipReason` — but **not** `BlockEntry` / `ContainerEntry` / `EntityEntry` / `EntryQueueRunner`. The package partition lies: the namespace hierarchy suggests a unified API surface but the module boundary leaves three Core-only classes floating in "API" space.

This mostly works until someone tries to publish `OmniscienceAPI` separately for external consumers. The listener-based `OmniCore.registerEventWrapperClasses` references `BlockEntry.class` etc. directly (`OmniCore.java:104-110`), which means Core can't compile without those classes, but the classes themselves pretend to be API-level.

### 5. `EntryQueue` is lossy on crash

`EntryQueue.getQueue()` is a static `LinkedBlockingDeque<DataWrapper>` (`EntryQueue.java:11`). `EntryQueueRunner` (`EntryQueueRunner.java:13`) is scheduled at `OmniCore.onEnable` for 20-tick intervals (`OmniCore.java:74-77`) — i.e. once per second. If the server crashes, SIGKILLs, or restarts within that one-second window, every queued entry is lost. `OmniCore.onDisable` (`OmniCore.java:95`) is empty — no flush-on-shutdown.
...
### 11. `OmniListener.handles` is `List.contains` — linear scan every event lookup

`OmniListener.java:24` is `events.contains(event)`. For a listener handling one event (the common case) that's fine. But `OmniEventRegistrar.enableEvents` (`OmniEventRegistrar.java:174`) filters `listeners.stream()` with that predicate for every event in the mapping — O(events × listeners) at startup. With 40 events and 30 listeners that's 1200 comparisons; not hot but not clean either. A `Map<String, OmniListener>` built once would be more honest.
...
```java
Thread.ofVirtual().name("omni-queue-drain-", 0).start(() -> {
    while (running) {
...
`OmniCore.onDisable` should drain the queue synchronously with a timeout:
```java
void onDisable() {
    shutdownFlag = true;
...
Kill `OmniCore.registerEventWrapperClasses` as a hard-coded switch. Move the mapping to the sealed type itself:
```java
...
```
The registry becomes `Map<String, Class<? extends DataEntry>>` populated by scanning the sealed permits list at startup. External plugins extend the sealed type via a second-tier `sealed interface ExtensionEntry extends DataEntry permits ...` declared in their plugin, registered via an `OmniscienceExtension` service.

### 7. Pluggable event-name → entry-class mapping (keep)

The current string-keyed dispatch (`OmniApi.getEventClass`, `OmniEventRegistrar.eventMapping`) is fine as a concept — it lets the wire format (Mongo's `Event` field) stay stable across refactors. Keep the external-facing string name, just make the Java side type-safe.

### 8. Generic `OmniListener` base with config-gated enable

Current `OmniListener.isEnabled(event)` does a lookup every time an event fires. Modern pattern: the generic dispatcher only registers with `PluginManager` when the event is enabled; toggling requires a re-register. Paper's `LifecycleEventManager` supports this. Kills the per-event `isEnabled("xyz")` call inside every listener method.
...
### 16. Collapse the `OmniEventRegistrar.getEventMapping` exception branch

Delete the 60-line anonymous-Map fallback at `OmniEventRegistrar.java:104-164`. Return the `ImmutableMap` directly or let the exception propagate.

## What v2 should keep

- **String-keyed event names.** They're the wire-format identity in Mongo documents and `/omni e:` parameter values. They've been stable across years of deploy. Replacing them with integer IDs or enum ordinals would break every existing record. Keep.
