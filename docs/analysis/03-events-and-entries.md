...
- **Listener hierarchy** (`the v1 core/src/main/java/net/medievalrp/v1/listener/`): 30 classes across `block/` (12), `item/` (7), `entity/` (4), `chat/` (2), `player/` (3), plus top-level `OmniListener`, `WandInteractListener`, `PluginInteractionListener`, `CraftBookSignListener`, `PermissionListener`.
...
- **`EntryQueueRunner`** (`EntryQueueRunner.java:10`) — `Runnable`. Drains the entire deque into a `List<DataWrapper>` every invocation, then hands the whole batch to `v1.getStorageHandler().records().write(...)`. Scheduled by `OmniCore.onEnable` at 20-tick intervals asynchronously.
