# Omniscience Dissection — 06: Rollback and Restore

Files covered:

- API: `OmniscienceAPI/src/main/java/net/medievalrp/omniscience/api/entry/Actionable.java`, `ActionResult.java`, `ActionableException.java`, `SkipReason.java`
- Core entry impls: `Omniscience-Core/src/main/java/net/medievalrp/omniscience/api/entry/BlockEntry.java`, `ContainerEntry.java`, `EntityEntry.java`
- Core command glue: `Omniscience-Core/src/main/java/net/medievalrp/omniscience/command/OmniCommands.java` (`runApplier`, `runUndo`)
- Core helper: `OmniscienceAPI/src/main/java/net/medievalrp/omniscience/api/util/DataHelper.java` (the deserialization spine — `unwrapConfigSerializable`, `dataWrapperToMap`, `unwrapValue`)
- Core dead code: `Omniscience-Core/src/main/java/net/medievalrp/omniscience/worldedit/FAWERollbackHandler.java`
- Core state holder: `Omniscience-Core/src/main/java/net/medievalrp/omniscience/OmniCore.java` (`lastActionResults`)

## Responsibility

Take a historical record of a change (a `DataEntry` with `ORIGINAL_BLOCK` / `NEW_BLOCK` / `BEFORE` / `AFTER` / `ENTITY` / etc. in its `DataWrapper`) and **reverse** it back into the live world — or **reapply** it forward. Also: remember the last batch of reversals per player, so `/omni undo` can un-reverse them.

This is the most write-heavy of Omniscience's runtime operations and the one most likely to tick-lag the server. It is the only part of the plugin that takes a query result and mutates the world; every other command surface is read-only.
...
2. **`OmniCommands.runApplier`** — the driver: query storage, iterate results, dispatch to `rollback()` or `restore()` based on sort direction, tally results, stash the batch for potential undo.
3. **`OmniCommands.runUndo`** — the parallel-universe implementation: walks the stashed `ActionResult` list in reverse, but does **not** go back through `Actionable`. It duck-types the transactions and mutates the world directly.
...
- **`BlockEntry`** (`BlockEntry.java:21`) — the big one. Handles `break`, `place`, `grow`, `form` per `OmniCore.registerEventWrapperClasses`. `rollback()` restores `ORIGINAL_BLOCK`; `restore()` applies `NEW_BLOCK` (or AIR if absent). Both delegate tile-entity restoration to `handleTileEntity(editState, parent)` which knows about containers, signs, banners, and jukeboxes.
...
```
`getActionablesLimit()` returns `10000` by default (`config.yml:limits.actionables`). The default `/omni search` limit is `limits.lookup.size=1000`, so rollback gets 10× the result ceiling. This is a guardrail — without a cap, a zealous `/omni rollback t:30d` would load every break event from the past month into memory, which on a busy server is millions of records. But the limit is a **silent truncation**: no warning to the user, no indication that the set was clipped, no follow-up paginated rollback. The 10001st block from the newest end of the query doesn't get rolled back, and nobody finds out.

**Step 5 — Iterate and actuate.** `OmniCommands.java:216-228`:
```java
...
```
Note the silent filter: non-`Actionable` entries are dropped without even a skip record. If a player ran `/omni rollback a:say` by mistake, the `for` loop produces an empty `actionResults` and the "0 reversals" message reveals nothing about *why* — the chat events loaded fine, they just weren't reversible.
...
**Step 6 — Tally, report, stash.** `OmniCommands.java:233-260`:
```java
...
Absent `NEW_BLOCK` means: the entry was a break event (nothing was placed), so "restore forward" means re-break the block → AIR. This is the right behavior for a break: breaking is idempotent, so `/omni restore` on a break entry just re-clears the block. **But it's wrong for a place entry where `NEW_BLOCK` somehow got dropped in serialization.** Current codepath can't tell the difference — both "break entry" (expected no `NEW_BLOCK`) and "place entry with corrupt data" (expected `NEW_BLOCK`, missing) hit the same AIR fallback. See Pain Points #6.
...
`OmniCommands.java:273-333`. The parallel-universe implementation.

**Step 1 — Player gate.** Console can't undo (`OmniCommands.java:274-277`). Reasonable: the stash is keyed by UUID.

**Step 2 — Fetch the stash.** `Omniscience.getLastActionResults(player.getUniqueId())` returns `Optional<List<ActionResult>>`. Empty → "You have no valid actions to undo" (`OmniCommands.java:279-283`).

**Step 3 — Reverse and walk.** `List<ActionResult> results = Lists.reverse(oResults.get());` — Guava's reverse view. Then a `for (ActionResult result : results)` loop at `OmniCommands.java:289-324`. For each result:
...
**Step 4 — Tally.** Same shape as `runApplier`: send `"X reversals(). Y skipped"`. The `reversals()` with the trailing parens is a typo in the format string (`OmniCommands.java:328`). User-facing.
...
|---|---|---|
| `NEWEST_FIRST` | `actionable.rollback()` | `/omni rollback ...` |
| `OLDEST_FIRST` | `actionable.restore()` | `/omni restore ...` |
...
`BlockEntry`, `ContainerEntry`, `EntityEntry`. Nothing else. This is the most consequential design constraint of the subsystem — and it's imposed by the registration table at `OmniCore.registerEventWrapperClasses` (`OmniCore.java:103-111`), which hard-codes seven events to three classes and drops everything else into `DataEntryComplete`. There is no public API for an external plugin to register a fourth `Actionable` subclass (see doc 03, pain point #8).
...
A user running `/omni rollback t:1h` on a griefing incident mid-server-war gets the blocks back but loses all the fire, all the pickups, all the bundles. The rollback is partial without any indication that it's partial.
...
The command alias determines sort, sort determines intent. If a future flag handler sets sort based on some other criterion (e.g. `-ord=asc` in a `/omni rollback` query, which would flip the sort to `OLDEST_FIRST` — perfectly parseable by `FlagOrder`), the rollback silently becomes a restore. No `Intent` enum distinguishes "I want to reverse history" from "I want to fast-forward history."
...
`OmniCore.java:34`: `private Map<UUID, List<ActionResult>> lastActionResults = Maps.newHashMap();`

Observations:

- **Not persisted.** Server restart → lost. Plugin reload → lost. `/omni rollback ...` then `/stop` → un-undoable.
...
The user types `/omni rollback p:alice r:20 t:30m` and the world changes. There's no preview, no "this will affect 347 blocks, confirm with `/omni rollback confirm`," no particle outline of affected positions. A wrong parameter (wrong name, too-wide radius, too-long time window) is a commit.

### 12. Container-undo path mutates the live inventory, not a snapshot

`OmniCommands.java:300-302`:
```java
if (location.getBlock().getState() instanceof Container container) {
    container.getInventory().setContents(inventory.getContents());
}
...
`OmniConfig` and the parser register `-drain`, which sets `Flag.DRAIN` on the session (`FlagDrain.java:29`). Nothing reads it. A search of the codebase turns up zero `Flag.DRAIN` consumers in any rollback or entry path. The user types `/omni rollback p:alice -drain` and nothing happens differently than without `-drain`.
...
No guardrail. A caller that forgets to check `applied()` first will NPE or log `"Skip Reason: null"`. The chat-dump loop in `runApplier` at `OmniCommands.java:247-251` gates correctly, but anything else reading an `ActionResult` has to remember.
...
If a subsequent `/omni undo` is attempted, the stash contains 3000 `ActionResult`s (only the successful ones were appended to the list). The 7000 un-rolled-back blocks are un-accounted for. The user has to re-run the rollback and hope the query returns a consistent result.
...
What this means in practice: rolling back a break of a chest uses `BlockEntry` which uses the snapshot-inventory path and goes through `handleTileEntity`. Rolling back a withdraw from the same chest uses `ContainerEntry` which writes to the live inventory and bypasses the block-state mechanism entirely. The two are not composable. A `/omni rollback` spanning both types of events on the same container applies them in the wrong order (because sort is NEWEST_FIRST and Mongo doesn't know to ordering them by "block-level first, then slot-level") and will get confused results.
...
`/omni rollback p:alice r:50 t:6h` on a busy server might affect tens of thousands of blocks. The current sync path tick-lags the server for seconds. A Paper 1.21 virtual-thread path:
```java
Thread.ofVirtual().name("omni-rollback-" + player.getUniqueId()).start(() -> {
...
`/omni rollback p:alice r:50 t:6h --preview` (or default to preview mode, require `--apply` to commit). Flow:
...
4. Send `"1,247 affected blocks. Type /omni confirm to apply, auto-cancels in 15s."`
5. Either `/omni confirm` fires the apply, or a scheduled task silently drops the pending operation.
...
Survives restarts. Supports an undo history stack: `/omni undo` pops the most recent; `/omni undo 3` pops the one before that. Keyed by player UUID (same as now) but also with a per-server scope if multi-server support lands.
...
`editState.update(false, false)` suppresses physics everywhere. Some rollbacks want physics (restoring a water pool wants water to re-spread). Some don't (restoring a piston-based farm mid-cycle shouldn't tick the pistons). Expose as a per-operation flag: `/omni rollback --physics` or `--no-physics` (default off).
