...
- Core command glue: `the v1 core/src/main/java/net/medievalrp/v1/command/OmniCommands.java` (`runApplier`, `runUndo`)
...
- Core state holder: `the v1 core/src/main/java/net/medievalrp/v1/OmniCore.java` (`lastActionResults`)
...
**Step 7 — Stash for undo.** `v1.addLastActionResults` delegates to `OmniCore.addLastActionResults` (`OmniCore.java:220-222`) which just does `this.lastActionResults.put(id, results)` into a `HashMap<UUID, List<ActionResult>>`. In-memory only; a server restart (or plugin reload) erases the entire map. Not persisted. One slot per player — running a second rollback overwrites the first; there's no undo history stack.
...
**Step 2 — Fetch the stash.** `v1.getLastActionResults(player.getUniqueId())` returns `Optional<List<ActionResult>>`. Empty → "You have no valid actions to undo" (`OmniCommands.java:279-283`).
