# Omniscience Dissection — 09: Utilities and Gaps

Files covered:

- **API utils** (`OmniscienceAPI/src/main/java/net/medievalrp/omniscience/api/util/`):
...
- **Test tree** (`Omniscience-Core/src/test/java/…`): `OmniCoreTest.java`,
  `DataWrapperTest.java`, `DataKeyTest.java`, `DataMapperTest.java`,
  `EntryConsumerTest.java`.
- **Build** (`build.gradle`, `OmniscienceAPI/build.gradle`,
  `Omniscience-Core/build.gradle`, `settings.gradle`, `gradle.properties`,
  `Omniscience-Core/src/main/resources/plugin.yml`).
- Cross-cutting: dead code and TODOs scattered across the rest of the tree
  (`OmniCore`, `DataWrapper`, `EventInventoryListener`, `FAWERollbackHandler`,
...
`net.medievalrp.omniscience.api.util` is the grab-bag. Seven top-level files and
...
imports `OmniscienceAPI` drags the bungee chat API with them.
...
- **`unwrapConfigSerializable`** (`DataHelper.java:69-94`). The read side of the
  ...
  with warning-logging via `OmniApi.warning`. This is the only way rollback /
...
- **`buildLocation`** (`DataHelper.java:160-168`). Builds the clickable
  ...
  `/omnitele <worldUuid> <x> <y> <z>`. This is the entry point for the
  `/omnitele` attack surface described in `05-commands-and-display.md` §"The
  `/omnitele` command is exposed." The method is also the only reason
...
```
getPageHeader(page, max) → "«Omniscience» §7((Page 1/3))"
subHeader(text)          → "§8§otext§r"
success(text)            → "«Omniscience» §atext"
error(text)              → "«Omniscience» §c (Error) §7text§r"
prefix()                 → "«§bOmniscience§a»"                  [public]
bonus(text)              → "§7text"
...
No validation on the `prefix` literal — the text `«Omniscience»` is hard-coded
...
enabled`. Used exclusively by `OmniEventRegistrar.eventMapping: Map<String,
PastTenseWithEnabled>`. The `enabled` field is mutable via the registrar
(`OmniEventRegistrar` toggles `enabled` when events are registered /
...
The JIT could be told with a `Log` here — one line of "Omniscience entity
...
|---|---|---|
| `OmniCoreTest.java` | 3 (`onEnable`, `onLoad`, `onDisable`) | 2 are empty-body, 1 calls `core.onLoad(mock)` (no-op) |
...
- **`OmniCoreTest`** (`OmniCoreTest.java:1-96`) has Mockito mocks set up for
  every `Omniscience` lifecycle dependency — `PluginCommand`, `PluginManager`,
  `Server`, `BukkitScheduler`, `Logger`, `StorageHandler` — but all of
  `@Before setup()` is commented out (`OmniCoreTest.java:48-55`). The three
  test methods instantiate `new OmniCore()` and then either do nothing or call
  the no-op `onLoad`. No assertions. `onEnable` test has its only real call
  commented out (`OmniCoreTest.java:61`: `//core.onEnable(omniscience,
...
dependencies (`Omniscience-Core/build.gradle:15-17`), which is all we'd need
...
- **`OmniCore.onEnable` — two empty integration `if` blocks**
  (`OmniCore.java:79-87`). Both check a config flag × plugin-enabled
  ...
  calling `onWorldEditStatusChange(true)` (`OmniCore.java:154-156`). Toggling
  ...
  "Pain points #1".
- **`OmniCore.onCraftBookStatusChange(boolean)`** (`OmniCore.java:172-174`):
  ...
  landed.
- **`OmniCore.onDisable`** (`OmniCore.java:95-97`): empty body. No storage
  ...
  #8".
- **`OmniCore.onLoad`** (`OmniCore.java:92-93`): empty body. Probably
...
- **`EventInventoryListener.debugEvent(InventoryClickEvent)`**
  ...
  debug paths in other listeners are gated on `Omniscience.logDebug` — this
  one just sits there. Either delete or promote to `@EventHandler(priority
  = LOWEST)` gated on `OmniConfig.isDebugEnabled()`.
...
- **`PermissionListener`** (`PermissionListener.java:1-23`): `@EventHandler
  ...
  (`omniscience`, `pluginCommand`). Entirely dead. Flagged in
...
- **`FAWERollbackHandler`** (`FAWERollbackHandler.java:32-215`): 215 lines of
  ...
  clean path is a flag on `/omni rollback`: `-we` or `--batch` to use FAWE
...
- **`api.interfaces.WorldEditHandler`** (`WorldEditHandler.java:1-8`): API
  ...
  `OmniApi.registerWorldEditHandler` (`OmniApi.java:114-115`), stored into
  `OmniCore.worldEditHandler` (`OmniCore.java:322-324`), and then
  **never read**. The field is write-only. No code ever pulls it out and
  ...
  orchestrated by `OmniCore.onWorldEditStatusChange` +
...
- **`AiHandler.reloadPrompt()`** (`AiHandler.java:60-62`): public method,
  called from nowhere. Presumably intended for a `/omni ai reload-prompt`
  subcommand that never shipped. Either wire it up (it's one line to add to
  `OmniCommands.register`) or delete.

### API surface dead bits

- **`ServerParameter`** (`OmniscienceAPI/…/api/parameter/ServerParameter.java`):
...
|---|---|---|
| Empty WE/FAWE integration ifs | `OmniCore.java:79-87` | Delete or wire to toggle |
| Empty `onCraftBookStatusChange` | `OmniCore.java:172-174` | Implement or delete |
| Empty `onDisable` | `OmniCore.java:95-97` | Add queue flush |
...
5. `OmniCore.java:173` — `//TODO turn off craft book related events if
...
```
# Compile-only local jars (e.g. FAWE.jar referenced by Omniscience-Core/build.gradle).
# Dev must drop a FAWE jar in Omniscience-Core/libs/ locally or swap to a maven dep.
Omniscience-Core/libs/
```
...
`Omniscience-Core/src/main/resources/plugin.yml:7`:

```yaml
website: https://github.com/itdontmatta/Omniscience
```
...
page we want players to hit. `https://medievalrp.net/plugins/omniscience`
or `https://github.com/medievalrp-net/Omniscience` are the natural
candidates.
...
`OmniCore.java:118`), so in practice the soft-depend is defensive only.
...
3. **Expanded into plugin.yml.** `processResources` does `expand(tokens)`
   to inject `${version}` into `plugin.yml` (`Omniscience-Core/build.gradle:
...
`OmniscienceAPI/build.gradle` is a `java-library` project with no publish
...
`rootProject.name = 'Omniscience'`. Fine.
...
(commons-lang3, OmniscienceAPI) don't have their licenses propagated
...
- The two empty `if` blocks in `OmniCore.onEnable:79-87`
- `Omniscience-Core/src/test/java/…/consumer/EntryConsumerTest.java`
- `Omniscience-Core/src/test/java/…/domain/DataMapperTest.java`
...
- **FAWE rollback** — real feature, real performance win for large
  rollbacks. Delete-or-finish the wiring: a `-fawe` flag on `/omni
...
- Switch version string to semver (`0.1.0-alpha.1`). Add gradle property
  ...
  ('omniVersion') ? project.property('omniVersion') : '0.1.0-SNAPSHOT'`.
...
`net.medievalrp.omniscience.internal.reflection` in the Core module,
...
5. Delete confirmed-dead code: `CommandResult`, `UseResult`,
   ...
   `debugEvent`, two empty integration ifs in `OmniCore.onEnable`,
