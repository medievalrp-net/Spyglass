# Omniscience Dissection — 00: Overview

## Module layout

Two Gradle subprojects:

- **`OmniscienceAPI`** — the public face. Declares interfaces, data types, builders, and helper utilities that external plugins (and Omniscience-Core itself) consume. Everything under `net.medievalrp.omniscience.api.*`. Ends up bundled inside the shaded core jar; published separately would be cleaner.
- **`Omniscience-Core`** — the plugin. Contains the Paper plugin entry point, config loader, storage adapters, event listeners, commands, WorldEdit/FAWE integration. Everything under `net.medievalrp.omniscience.*` and the nested `net.medievalrp.omniscience.api.*` (yes — Core puts parameter/flag *implementations* under the `api` package root even though it's Core-side code; this is a historical mess that muddies the API/impl boundary).
...
| Area | Files | Notes |
|---|---|---|
| Plugin bootstrap | `Omniscience.java`, `OmniCore.java`, `OmniEventRegistrar.java`, `OmniConfig.java` | Entry + config |
...
| External APIs | API: `OmniApi`, `IOmniscience`, `WorldEditHandler` | Public surface for other plugins |
...
Collection: `DataEntry` inside database `Omniscience` (configurable). Each document is a flat representation of a `DataWrapper`:
...
- Shadowing: `shadow 9.0.0-beta12`; includes `:OmniscienceAPI` + `commons-lang3` into the final Core jar
- plugin.yml token expansion via `processResources` + `${version}` substitution
- compileOnly deps: mongo driver, paper-api, worldedit-bukkit, cloud-paper, local FAWE jar (see `Omniscience-Core/libs/`, gitignored)
...
1. **API/impl boundary leak.** The API module declares `DisplayHandler`, `FlagHandler`, `ParameterHandler` *interfaces*, but Omniscience-Core puts concrete `Parameter*` and `Flag*` classes under `net.medievalrp.omniscience.api.parameter.*` / `.flag.*` — same package root as the API. A consumer plugin that imports OmniscienceAPI gets the interfaces only, but the package layout pretends otherwise.
...
- `01-core-lifecycle.md` — `Omniscience.java`, `OmniCore.java`, `OmniEventRegistrar.java`, `OmniConfig.java`
...
- `08-api-surface.md` — API: `OmniApi`, `interfaces/IOmniscience`; the effective consumer contract
