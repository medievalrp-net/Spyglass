...
Files covered: `v1.java`, `OmniCore.java`, `OmniEventRegistrar.java`, `OmniConfig.java`, `src/main/resources/plugin.yml`, `src/main/resources/config.yml`.
...
- **`v1`** — `JavaPlugin` subclass. Holds two statics: `INSTANCE` (`OmniCore`) and `PLUGIN_INSTANCE` (`v1` itself). Entire plugin's public surface to the rest of the code reaches through static methods on this class (`v1.getStorageHandler()`, `v1.hasActiveWand()`, etc.). Every static delegates to the corresponding `OmniCore` instance method.
- **`OmniCore`** — the actual plugin state. Implements `Iv1`. Holds:
...
- **`OmniConfig`** — singleton (`OmniConfig.INSTANCE`). Reads `config.yml` and exposes typed getters for: DB type and name, wand material, debug flag, database limits, defaults (radius, time), integration toggles (FAWE, CraftBook, WorldEdit), AI settings, record expiry, pool size, max radius, etc. Also defines a `DatabaseType` inner enum mapping `"mongodb"`/`"dynamodb"` strings to `StorageHandler` subclass constructors via reflection (`invokeConstructor()`).
- **`OmniEventRegistrar`** — another singleton (`OmniEventRegistrar.INSTANCE`). Owns the `Map<String, PastTenseWithEnabled>` of every registerable event (e.g., `"break"` → `PastTense("broke", enabled=true)`) and the list of `OmniListener` instances. `enableEvents()` actually registers them with Bukkit's `PluginManager` (only if the event is enabled in config).
...
1. **Dead integration if-blocks.** Lines 79–87 of `OmniCore.onEnable` read config booleans for `fastAsyncWorldEdit` and `worldEdit` integration, check whether the plugin is enabled, and then do nothing inside the if-blocks. WorldEdit wiring actually happens inside `registerFlags()` via `onWorldEditStatusChange(true)` at line 154 — not via the integration config toggles. So turning `integration.worldEdit: false` in config doesn't actually disable anything; the checks are half-wired.
...
3. **Two singleton classes.** `v1` holds two statics (`INSTANCE`, `PLUGIN_INSTANCE`). `OmniConfig.INSTANCE`. `OmniApi` holds a core reference. `OmniEventRegistrar.INSTANCE`. Five separate singleton patterns for one plugin.
4. **`OmniCore` implements `Iv1` but isn't fully public.** The class is package-private (`final class OmniCore`) — it can only be reached via `OmniApi.getCore()` (also package-private) or the static delegates on `v1`. `Iv1` is the cleaner interface but code inside Core still calls `OmniCore`-specific package-private methods, so the interface contract isn't enforced.
...
1. **Kill singletons, use constructor injection.** `OmniCore` takes a `StorageHandler`, `ConfigProvider`, `EventRegistry`, etc. via its constructor. `v1` (the Paper plugin) builds them. Nothing needs a static accessor. Makes testing possible.
