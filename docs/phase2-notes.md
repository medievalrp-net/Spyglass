...
Phase 2 block 1 — command layer refactor (services + thin SpyglassCommands)
...
### Commands
`/sg help`, `/sg events`, `/sg search <params>`, `/sg rollback <params>`, `/sg restore <params>`, `/sg undo`, `/sg page <n>`, `/sg tool` (placeholder), aliases `/o2` and `/spyglass`.

Command registration consolidated to `SpyglassCommands.register()` — a single method where every handler is a one-liner delegate. Business logic lives in named services under `command/service/`.
...
3. **v1 → v2 migration tool.** `/sg admin migrate-v1` console command reading `v1.DataEntry` + translating to v2 records. Essential for server operators upgrading from v1.
4. **Inspection wand** (`/sg tool` proper implementation). Toggle persists in Mongo; right-click a block runs a location-scoped search.
...
- RCON synchronous-reply model can't deliver async query results inline. Search via RCON shows "Searching..."; results land in the page cache and are retrieved via `/sg page 1`. In-game chat is fine (messages deliver as they arrive).
