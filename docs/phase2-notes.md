...
### Commands
`/omniv2 help`, `/omniv2 events`, `/omniv2 search <params>`, `/omniv2 rollback <params>`, `/omniv2 restore <params>`, `/omniv2 undo`, `/omniv2 page <n>`, `/omniv2 tool` (placeholder), aliases `/o2` and `/omniscience2`.
...
### Regression harness
`./gradlew regression` seeds both `Omniscience.DataEntry` (v1) and `Omniscience2.EventRecords` (v2) via pymongo with deterministic test data, then runs the query matrix in `regression/cases.json` through RCON. Current matrix: 21 cases, all green. Report lands at `plugin/build/reports/regression/report.json`.
...
3. **v1 → v2 migration tool.** `/omniv2 admin migrate-v1` console command reading `Omniscience.DataEntry` + translating to v2 records. Essential for server operators upgrading from v1.
